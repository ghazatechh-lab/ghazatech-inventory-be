        ...
        if purchase_order:
            if purchase_order.status not in {"APPROVED", "PARTIALLY_RECEIVED"}:
                raise serializers.ValidationError(
                    {
                        "purchase_order": (
                            "GRN can only be created for an approved purchase order."
                        )
                    }
                )

            # Do not allow the GRN API to be bypassed by manually posting a PO
            # id. The linked PO must have a shipment that has actually been
            # received/confirmed first.
            has_confirmed_shipment = purchase_order.shipments.filter(
                status__in=["RECEIVED", "COMPLETED"]
            ).exists()
            if not has_confirmed_shipment:
                raise serializers.ValidationError(
                    {
                        "purchase_order": (
                            "Confirm/receive the purchase shipment before creating a GRN."
                        )
                    }
                )
...
class PurchaseExpenseSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if self.instance and str(self.instance.status or "").upper() == "PAID":
            raise serializers.ValidationError(
                {
                    "status": "Paid expenses cannot be edited. Create a correcting entry instead."
                }
            )
        ...
        if method in ["BANK_TRANSFER", "CHEQUE", "CARD"] and not bank:
            raise serializers.ValidationError(
